let vendedor: string[] = new Array(5);
vendedor = ['Camila', 'Franco', 'Sofia', 'Matias', 'Agustina'];

let ventasSemanales: number[][] = new Array(5);
...
function ventaMaximaSemanal(matriz: number[][], fila): number {
    let mejorSemana: number = 0;
    for (let semana: number = 0, semana < matriz[fila][])
    return mejorSemana
}