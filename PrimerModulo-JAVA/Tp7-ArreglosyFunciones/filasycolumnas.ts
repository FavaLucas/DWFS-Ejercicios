...
let arregloSimple: number[] = new Array(10);
...
//Muestra en pantalla el arreglo de 10x10
for (let i: number = 0; i < filas.length; i++) {
    console.log(filas[i].join('    '));
}

//comienzo a jugar
let sigueJuando: boolean = true;
while (sigueJuando) {
    let fila: number = rls.questionInt("Ing fila: ");
    let columna: number = rls.questionInt("Ing columna: ");

    let valorSeleccionado = filas[fila][columna];
    ...
    //
    let encontreUnUno: boolean = false;
    for (let i: number = 0; i < filas.length; i++) {
        const columnas = filas[i];
        for (let j: number = 0; j < columnas.length; j++)
            if (columnas[j] == 1) {
